...
%         GridLayout
    ...
    methods
        function app = plottingToolsUI(simulation) 
            ...
            % Buttons
            app.addButton('Plot 1D',  {@app.openPlotUI, '1D'})
            app.addButton('Plot 2D',  {@app.openPlotUI, '2D'})
...
%             app.addButton('Plot detection range polarplot',   {@app.plot2D, 'DRPP'})
    ...
    %% Plot functions
    methods
        function openPlotUI(app, hObject, eventData, type)
            isLoaded = checkSimulationIsLoaded(app);
            if isLoaded
                switch type
                    case '1D'
                        app.subWindows{end+1} = plot1DUI(app.Simulation);
                    case '2D'
                        app.subWindows{end+1} = plot2DUI(app.Simulation);
                end
            end
        end
    end
